...
# st.session_state is Streamlit's way of storing variables that persist between interactions
...
# --- Dashboard Plotting Function (Same logic as before) ---
def plot_dashboard():
    # ... (The plotting logic is identical to your Jupyter version)
    ...
    if total_income > 0:
        ...
        axs[0].text(0.5, 0.5, 'No Income Data', ha='center', va='center'); axs[0].axis('off')
    ...
    if total_expense > 0:
        ...
        axs[1].text(0.5, 0.5, 'No Expense Data', ha='center', va='center'); axs[1].axis('off')
    ...
    if total_income > 0:
        ...
        if display_expense > 0 or savings > 0:
            axs[2].pie([display_expense, savings], labels=[f'Expenses (${display_expense:,.2f})', f'Savings (${savings:,.2f})'], autopct='%1.1f%%', startangle=90, colors=['#ff9999','#66b3ff'])
        axs[2].set_title('Income Allocation', fontsize=14)
    else:
        axs[2].text(0.5, 0.5, 'No Income Data', ha='center', va='center'); axs[2].axis('off')
...
with col1:
    ...
    if st.button("💾 Save Transaction"):
        ...
        if st.session_state.editing_index is not None:
            ...
            st.session_state.editing_index = None # Exit edit mode
        else:
            st.session_state.transactions.append(new_transaction)
            st.success("Transaction added successfully!")
        st.experimental_rerun() # Refresh the page to show changes
